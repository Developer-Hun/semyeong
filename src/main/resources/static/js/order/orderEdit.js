const orderEdit = {
    ...
    getItems: () => {
        if (orderEdit.items == undefined) {
            orderEdit.getItemsRequest();
            return;
        }
        if (orderEdit.items.length == 0) return alert ("현재 시스템에 등록된 품목이 없습니다.");

        orderEdit.addManagementItemRow();
    },
    ...
    setItemInfo: (selectedTarget) => {
        ...
        const findItem = orderEdit.items.find((item) => item.id == selectedId);
    ...
    addManagementItemRow: () => {
        ...
        const options = orderEdit.items.map((item, i) => {
        ...
        let row =  `<tr class="item-row">
                        ...
                        <td>
                            <select class="select2_single form-control" tabIndex="-1" name="item" onchange="orderEdit.setItemInfo(this)">
    ...
    deleteManagementItemRow: () => {
        ...
        orderEdit.deleteManagementItemRequest();
    },

    getItemsRequest: () => {
        const successHandler= (data) => {
            orderEdit.items = data;
            orderEdit.addManagementItemRow();
        }
